...
parser.add_argument('--tau', type=float, nargs='+', help='temperature for distillation')
...
class model_teacher(nn.Module):
    def __init__(self):
        ...
        self.load_finetune_checkpoint('convnext-base-4expert-90.81.ckpt')
...
class model_top(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for cnt_moe in range(4):
            pred = self.moe_head[cnt_moe](x[:, cnt_moe * 192:cnt_moe * 192 + 192])
            loss += F.kl_div(F.log_softmax(pred / tau[cnt_moe], dim=1),
                             F.log_softmax(teacher_out / tau[cnt_moe], dim=1),
                             reduction='sum',
                             log_target=True) * (tau[cnt_moe] * tau[cnt_moe]) / pred.numel() / 4
            preds += pred
